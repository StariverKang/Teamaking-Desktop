      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {Array.isArray(profile?.outputTags)
              ? profile.outputTags.slice(0, 4).map((tag: string) => <SkillBadge key={tag}>{tag}</SkillBadge>)
              : null}
            {(user?.skills ?? []).slice(0, 5).map((item: any) => (
              <SkillBadge key={item.id ?? item.skill?.name}>{item.skill?.name ?? item.name}</SkillBadge>
            ))}
          </div>
        </div>
      </div>
...
export function TeamakingPostCard({ post }: { post: any }) {
  ...
  const personalTags = Array.isArray(profile?.outputTags) ? profile.outputTags : [];
  ...
  return (
    <Card>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {personalTags.slice(0, 4).map((item: string) => (
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {Array.isArray(post.strengths)
          ? post.strengths.map((item: string) => <SkillBadge key={item}>Signal · {item}</SkillBadge>)
          : null}
      </div>
      <div className="mt-3 flex flex-wrap gap-2">
        {Array.isArray(post.contributionTypes)
          ? post.contributionTypes.map((item: string) => (
          <span key={item} className="border border-ink/25 px-2.5 py-1 text-xs text-ink/68">
                {item}
              </span>
            ))
          : null}
