...
当前产品边界分成两条互补数据线：每年 admission programme handbook 负责学生入学届别、专业培养方案推荐和 CourseBoard 默认加入规则；course catalog 负责学校级课程总表，合并 Course Descriptions、University Core、General Education 三类来源。Course catalog 不绑定 semester、年级或专业，批准后所有 active 课程都可被学生搜索并手动打开 Course Board。BNBU class schedule 只是学期时间安排，不作为课程存在、真实开课或 CourseBoard 配置依据；`semester_offerings` 与 `syllabus_teamwork` 不属于当前必要管线。Handbook import 可以保持 `offerings[]` 为空，系统会通过当前 academic term、学生 admission year、major 和 `relativeTermCodes` 激活 CourseBoard。
...
Course catalog 的 cleaned JSON 是学校级课程库快照，形状应类似：
...
- Course catalog JSON 必须使用 `importMode: "course_catalog"` 和 `catalogEffectiveYear` 表示课程目录版本；不要写入 `semester`，不要生成专业、年级或 admission-year rules。
...
- Missing `semester.code`, `semester.name`, `semester.academicYear`, or `semester.term` for non-`course_catalog` imports.
- Missing `catalogEffectiveYear` for `course_catalog` imports.
- Empty `faculties` for non-`course_catalog` imports.
- Empty `courses`.
- Empty `offerings` for v1; v2 may warn instead when importing handbook-only curriculum data.
- `course_catalog` contains `semester`, `majors`, `offerings`, or `curriculumRules`.
...
- Course catalog JSON can go through workflow create pending / approve and produce searchable course rows with source refs from Course Descriptions, University Core, and General Education while leaving `semester`, `offerings[]`, and `curriculumRules[]` empty.
