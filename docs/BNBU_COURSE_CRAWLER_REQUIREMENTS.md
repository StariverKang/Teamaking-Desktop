...
本文档用于指导外部开发者实现 BNBU 课程数据采集与处理程序。TEAMAKING 不直接爬取 BNBU 官网；外部程序负责采集、保留原始证据、清洗结构化数据，并最终输出 TEAMAKING 可导入的业务 JSON。
...
最终 JSON 会由管理员在 TEAMAKING `/admin/course-imports` 上传、校验、创建待审批批次，并在人工批准后写入正式数据库。
...
- 年级/学期识别：把 `Year 2`, `Y2`, `Second Year` 统一为 `Year 2`；把 Fall/Semester 1 等归一到目标 semester。
...
最终给 TEAMAKING 的文件必须是一个 JSON object，不是 JSONL。schema version 固定：
...
- `grades`: match `Year 1`, `Year 2`, `Year 3`, `Year 4`.
...
- `schemaVersion` is not `teamaking.bnbu_course_import.v1`.
