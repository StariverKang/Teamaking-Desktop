...
| 自然语言说明 | `instruction` | 文本 | 否 | 一段默认说明 | `爬取 2025 和 2024 admission 的 programme handbook。` | 辅助自动识别 URL、admission years、programme codes、limit 和 target。 | 明确字段会覆盖自然语言推断。自然语言适合快速填写，但正式任务建议同时检查下方结构化字段。 |
...
| Course catalog URL | `courseDescriptionsUrl` | URL | `course_catalog` 需要 | BNBU Course Descriptions 页面 | `https://ar.bnbu.edu.cn/info/1021/1430.htm` | Crawler 从这里找到 Course Descriptions PDF，并自动补入 University Core 与 General Education 官方 PDF，生成课程总表 JSON。 | Course catalog 是学校级课程总表来源，不生成 semester、major、grade 或 admission-year curriculum rules。 |
...
| Catalog effective year | `catalogEffectiveYear` | 年份数字 | `course_catalog` 需要 | `2026` | `2026` | 写入 course catalog lifecycle metadata，用于判断课程描述版本新旧。 | 不创建 semester，不表示开课学期，也不限制哪些学生、年级或专业可以搜索/打开这门课。 |
...
| Course catalog | `course_catalog` | 已支持 | BNBU AR Course Descriptions、University Core、General Education 三类课程总表来源，补充 `courses[]` 和 source refs，不生成专业、semester、offerings 或 admission-year rules。 |
...
| 直接批准并更新线上数据库 | `approve_import` | 创建导入批次后立即批准；course catalog 只更新 Course，programme handbook 更新 Course 与 CourseCurriculumRule；完整版本 checkpoint 由管理员在版本管理页手动创建。 | 现场确认范围无误后快速更新线上数据库。 |
...
| 派生字段 | 来源 | 示例 | 说明 |
|---|---|---|---|
| `catalogEffectiveYear` | 页面字段或旧版 `academicYear` 兼容字段 | `2026` | 只用于 course catalog lifecycle 对比；不是 semester。 |
| `semesterCode` | handbook admission plan 上下文 | `2025-Admission-Programme-Plan` | 仅 programme handbook JSON 使用，用来承载 admission-year curriculum rules；course catalog 不写入。 |
...
| Catalog / plan | Course catalog 显示 catalog effective year；programme handbook 显示 admission-year plan。 | Catalog effective year 是课程目录版本，不是学期；programme handbook plan 是培养方案规则上下文。 |
...
| 问题 | 现象 | 正确理解 / 处理 |
|---|---|---|
| 把 `Catalog effective year` 当开课学期 | 以为 course catalog 把课程绑定到某个 Spring/Fall。 | Catalog effective year 只表示课程描述版本；course catalog 不生成 semester、major、grade 绑定。 |
| 把 programme handbook 规则当成课程可见性限制 | 只看 Recommended tab，以为其他专业或年级不能上这门课。 | Handbook rules 只影响推荐/默认加入；所有 active course catalog 课程都能搜索并手动打开 Course Board。 |
...
- Course catalog JSON 不应包含 `semester`；只检查 `catalogEffectiveYear` 是否是本次课程目录版本。
- Programme handbook JSON 的 `semester.code` 是否是 admission plan 上下文，例如 `2025-Admission-Programme-Plan`。
- Programme handbook JSON 的 `semester.isCurrentCandidate` 是否为 `false`。
