...
本项目里的 Friend 不单独建聊天关系表。只要 Follow Request 进入 `accepted`，双方就被视为好友，并会出现在 `/friends`。

Friend 关系目前用于列表展示、Profile 入口和后续可见性扩展；它不是私信聊天系统。

### Course Review Comment

真实 `Course` 目录下的课程评价或回复。它绑定课程目录，不绑定某个 semester offering 或 Course Board。

Course Review Comment 支持多级回复、真实时间和软删除；被删除后保留楼层结构，正文显示“评论已删除”。不支持点赞或匿名差评。

### Content Document

管理员维护的 Markdown 文档，用于帮助中心、开发者日志和联系开发者页面。文档可以有父子级、发布/隐藏状态、排序和最多三张开发者日志图片。

Content Document 不是用户可编辑内容；它属于系统框架内容，会随管理员发布同步到用户侧。
...
- 说 “Friends” 时，只指 accepted Follow Request 形成的 mutual follow 列表。
- 说 “Course Review” 时，只指真实课程目录下的评价/回复，不指 Course Board 帖子。
